...
export async function getLatestScan() {
  const resp = await fetch(`${API_BASE}/api/scan/latest`, { headers: authHeaders() });
  return handle(resp);
}
export async function getHistory(limit = 30) {
  const resp = await fetch(`${API_BASE}/api/scan/history?limit=${limit}`, { headers: authHeaders() });
  return handle(resp);
}
export async function getScanDetail(id) {
  const resp = await fetch(`${API_BASE}/api/scan/${id}`, { headers: authHeaders() });
  return handle(resp);
}
...
export async function getMarketContext(refresh = false) {
  const resp = await fetch(`${API_BASE}/api/context?refresh=${refresh}`, { headers: authHeaders() });
  return handle(resp);
}
export async function getWatchlist() {
  const resp = await fetch(`${API_BASE}/api/watchlist`, { headers: authHeaders() });
  return handle(resp);
}
...
export async function getBacktestStats(category, period) {
  ...
  const resp = await fetch(`${API_BASE}/api/backtest/stats${qs}`, { headers: authHeaders() });
  return handle(resp);
}
export async function getRecentOutcomes(limit = 20, category, period) {
  ...
  if (category) params.set("category", category);
  if (period) params.set("period", period);
  const resp = await fetch(`${API_BASE}/api/backtest/recent?${params.toString()}`, { headers: authHeaders() });
  return handle(resp);
}
export async function getBacktestCategories() {
  const resp = await fetch(`${API_BASE}/api/backtest/categories`, { headers: authHeaders() });
  return handle(resp);
}
export async function getConfig() {
  const resp = await fetch(`${API_BASE}/api/config`, { headers: authHeaders() });
  return handle(resp);
}
